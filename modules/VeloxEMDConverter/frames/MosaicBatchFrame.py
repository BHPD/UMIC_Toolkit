...
class MosaicBatchFrame(customtkinter.CTkFrame):
    def __init__(self, master):
        ...
        self.info=customtkinter.CTkLabel(
            self, 
            text='Process multiple EMD mosaics' +
            '\nExports elemental maps as generated by Velox from a folder of folders with .emd files with global normalization' + 
            '\nNote all .emd files should have the same elements selected',
            anchor='center')
        ...
        self.file_input=customtkinter.CTkLabel(
            self,
            text='Input folder containing sub-folders with the individual mosaics',
            anchor='w')
       ...
       # Row 4, normalize option
        ...
        self.options_norm.grid(row=4, column=1, sticky='ew', pady=5, padx =5)
        
        # Row 4, normalize minimum parameter - default 0.5%
        self.options_norm_min=customtkinter.CTkEntry(
            self,
            placeholder_text='Min (Default 0.5%)')
        self.options_norm_min.grid(row=4, column=2, sticky='ew', pady=5, padx =5)

        # Row 4, normalize maximum parameter - default 99.5%
        self.options_norm_max=customtkinter.CTkEntry(
            self,
            placeholder_text='Max (Default 99.5%)')
        self.options_norm_max.grid(row=4, column=3, sticky='ew', pady=5, padx =5)
        
        # Row 5, Gaussian blur option
        self.options_blur=customtkinter.CTkCheckBox(
            self,
            text='Gaussian blur',
            command=lambda: self.force_normalize()) # trigger: if checked, force normalize ON
        self.options_blur.grid(row=5, column=1, sticky='ew', pady=5, padx =5)

        # Row 5, Gaussian blur sigma parameter - default 1
        self.options_blur_sigma=customtkinter.CTkEntry(
            self,
            placeholder_text='Sigma (Default 1)')
        self.options_blur_sigma.grid(row=5, column=2, columnspan=2, sticky='ew', pady=5, padx =5)

        # Row 6, invert option
        self.options_invert=customtkinter.CTkCheckBox(
            self,
            text='Invert HAADF')
        self.options_invert.grid(row=6, column =1, sticky='ew', pady =5, padx=5)
    ...
    def force_normalize(self):
        if self.options_blur.get() == 1 or self.options_dtype.get() == '8-bit':
            ...
            self.haadf_norm.configure(state='normal')

    def force_default(self):
        if self.options_norm.get() == 0:
            self.haadf_norm.configure(state='disabled')
