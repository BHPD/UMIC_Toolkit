...
class SingleNormalFrame(customtkinter.CTkFrame):
    def __init__(self, master):
        super().__init__(master)

        # Initialisation
        self.file_input_path=None # Placeholder for the input_path
        ...
        self.info=customtkinter.CTkLabel(
            self, 
            text='Process individual EMDs' +
            '\nExports elemental maps as generated by Velox',
            anchor='center')
    ...
    def browse_export(self):
        start_dir=self.file_input_path
        if start_dir and os.path.isfile(start_dir):
            start_dir=os.path.dirname(start_dir)
        elif not start_dir or not os.path.isdir(start_dir):
            start_dir=os.getcwd()
            
        folder=filedialog.askdirectory()
        if folder:
            self.file_export_path=folder
            self.file_export.configure(text=folder)

    def convert(self):
        start_time = time.time()
        def task():
            self.progress.start()
            bf.processSingleNormal(self,
